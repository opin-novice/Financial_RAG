...
def process_single_file(file_path: Path, model, collection) -> int:
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        for j, chunk in enumerate(batch_chunks):
            ...
            # Calculate page number based on chunk position
            # Assuming roughly 500 words per page for text files
            words_before_chunk = sum(len(chunks[k].split()) for k in range(i + j))
            estimated_page = max(1, (words_before_chunk // 500) + 1)
            
            metadata = {
                'source': file_path.name,
                'chunk_id': i + j,
                'page_number': estimated_page,  # Add this line
