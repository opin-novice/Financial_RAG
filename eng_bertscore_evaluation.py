#!/usr/bin/env python3
"""
BERTscore Evaluation for ENGqapairs - Vanilla vs Advanced RAG
============================================================
Focused evaluation script for English question-answer pairs
"""
...
class ENGqapairBertScoreEvaluator:
    """Focused BERTscore evaluator for ENGqapairs"""
    
    def __init__(self, timeout_seconds: int = 60, max_questions: int = 10):
        """
    ...
    def evaluate_engqapairs(self) -> Dict:
        """
        Evaluate ENGqapairs dataset with both RAG systems
        
        Returns:
            Dictionary containing evaluation results
        """
        print("="*80)
        print("BERTSCORE EVALUATION FOR ENGQAPAIRS")
        print("="*80)
        ...
        if self.max_questions:
            eng_questions = eng_questions[:self.max_questions]
            print(f"[INFO] Processing {len(eng_questions)} questions (limited to {self.max_questions})")
        else:
            print(f"[INFO] Processing all {len(eng_questions)} questions")
        
        results = {
            'dataset_name': 'ENGqapairs',
            'total_questions': len(eng_questions),
            'vanilla_rag': {'responses': [], 'references': [], 'successful': 0, 'failed': 0},
            'advanced_rag': {'responses': [], 'references': [], 'successful': 0, 'failed': 0}
        }
        
        # Process each question
        for i, qdata in enumerate(eng_questions):
            question = qdata['question']
            ground_truth = qdata.get('answer', '')
            
            print(f"\n[INFO] Processing question {i+1}/{len(eng_questions)}: {question[:60]}...")
        ...
        print(f"\n[INFO] Calculating BertScore metrics for ENGqapairs...")
    ...
    def print_research_results(self, results: Dict):
        ...
        print(f"\n📊 ENGqapairs Dataset Results:")
        print("-" * 50)
        
        # Vanilla RAG results
        vanilla = results['vanilla_rag']
...
def main():
    """Main function"""
    print("="*80)
    print("BERTSCORE EVALUATION FOR ENGQAPAIRS - VANILLA VS ADVANCED RAG")
    print("="*80)
    
    try:
        # Initialize evaluator with longer timeout and limited questions
        evaluator = ENGqapairBertScoreEvaluator(timeout_seconds=60, max_questions=5)
        
        # Run evaluation
        results = evaluator.evaluate_engqapairs()
        ...
        output_file = f"eng_bertscore_evaluation_{timestamp}.json"
