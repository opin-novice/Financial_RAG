#!/usr/bin/env python3
"""
Inference Time Measurement Script for Vanilla RAG
==================================================
Measures only the inference time (time to produce answers) for 5 questions
from Policy_Related.json using main.py (VanillaRAGSystem)
"""
...
# Import RAG system from main.py
...
def main():
    ...
    print("="*70)
    print("Inference Time Measurement Script - Vanilla RAG")
    print("="*70)
    ...
    for i, qdata in enumerate(questions_data, 1):
        ...
        results.append({
            ...
            'num_docs_retrieved': result.get('num_docs', 0)
        })
        
        print(f"   Inference Time: {inference_time:.2f} seconds")
        print(f"   Response Length: {len(result.get('response', ''))} characters")
        print(f"   Documents Retrieved: {result.get('num_docs', 0)}")
    ...
    for r in results:
        ...
        print(f"  Documents Retrieved: {r['num_docs_retrieved']}")
    
    # Save results to JSON
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"inference_time_vanilla_results_{timestamp}.json"
    ...
    output_data = {
        ...
        'average_inference_time_seconds': round(total_time/len(results), 2),
        'min_inference_time_seconds': round(min(r['inference_time_seconds'] for r in results), 2),
        'max_inference_time_seconds': round(max(r['inference_time_seconds'] for r in results), 2),
        'results': results
    }
