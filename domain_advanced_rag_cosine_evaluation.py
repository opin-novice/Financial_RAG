...
def get_advanced_rag_responses(questions, domain_name):
    """Get responses from Advanced RAG system"""
    try:
...
def print_summary_stats(similarity_results, total_questions):
    ...
    print("\n" + "="*60)
    print("📊 DOMAIN EVALUATION SUMMARY - ADVANCED RAG (COSINE SIMILARITY)")
    print("="*60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Domain-Based Advanced RAG Cosine Similarity Evaluation')
    parser.add_argument('--domain_file', required=True, help='Domain JSON file name (e.g., "Comparative_Financial.json")')
    args = parser.parse_args()
    
    domain_name = args.domain_file.replace('.json', '')
    
    print(f"🚀 Starting Advanced RAG Domain-Based Cosine Similarity Evaluation")
    ...
    # Get Advanced RAG responses
    print(f"\n�� Getting Advanced RAG responses...")
    responses = get_advanced_rag_responses(questions, domain_name)
    ...
    output_file = f"advanced_rag_domain_cosine_{domain_name}_{timestamp}.json"
