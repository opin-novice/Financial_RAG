#!/usr/bin/env python3
"""
Vanilla RAG Test Selector - Interactive JSON File Selection for Vanilla RAG Evaluation
====================================================================================
Allows you to select any JSON file from the dataqa folder to test main.py (Vanilla RAG)
and produces evaluation results similar to cosev3.py and delmax.py
"""
...
# Import your Vanilla RAG system
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'{LOGS_FOLDER}/vanilla_rag_test_selector.log', encoding='utf-8'),
...
class VanillaRAGTestSelector:
    """Interactive Vanilla RAG testing with JSON file selection"""
    
    def __init__(self):
        """Initialize the test selector"""
        logger.info("🚀 Initializing Vanilla RAG Test Selector...")
        self._init_embedding_model()
        self._init_rag_system()
        logger.info("✅ Vanilla RAG Test Selector initialized successfully")
    ...
    def _init_rag_system(self):
        """Initialize the Vanilla RAG system"""
        try:
            ...
            if not hasattr(self.rag_system, 'process_query') or not callable(self.rag_system.process_query):
                raise Exception("Vanilla RAG system does not have process_query method")
    ...
    def get_rag_response(self, question: str) -> Tuple[str, Dict]:
        """Get LLM response for a question using the Vanilla RAG system"""
        try:
    ...
    def run_evaluation(self, ground_truth_file: Path, max_questions: int = None) -> str:
        ...
        print(f"\n🔍 Running Vanilla RAG evaluation on: {ground_truth_file.name}")
        ...
        output_file = f"{LOGS_FOLDER}/vanilla_rag_test_{filename_base}_{timestamp}.json"
    ...
    def print_results(self, stats: Dict[str, float], file_path: str, total_questions: int, successful: int, failed: int):
        ...
        print("\n" + "="*60)
        print("📈 VANILLA RAG EVALUATION RESULTS")
...
def main():
    parser = argparse.ArgumentParser(description="Interactive Vanilla RAG Test Selector")
    ...
    try:
        selector = VanillaRAGTestSelector()
        ...
        print(f"\n🎉 Vanilla RAG evaluation completed successfully!")
